...
class RewardOrLinkedValidator:
    """Проверка на запрет одновременного
    заполнения полей reward и linked_habit."""
    ...
    def __call__(self, value):
        if value.get("reward") and value.get("linked_habit"):
            raise ValidationError(
                "reward и linked_habit одновременно заполнять нельзя"
            )
...
class PeriodicityValidator:
    ...
    def __call__(self, value):
        ...
        if periodicity and periodicity > 7:
            raise ValidationError(
                "Неверное значение частоты выполнения привычки. Нельзя выполнять привычку реже, чем 1 раз в 7 дней."
            )
